...
import infiapp.envento.photorecoverynew.adapter.OtherAdapter;
import infiapp.envento.photorecoverynew.task.RecoverOthersAsyncTask;
...
public class OtherActivity extends AppCompatActivity {
    ...
    private void gotonext() {
        ...
        if (status.equals("restore")) {
            ...
            if (tempList.size() == 0) {
                Toast.makeText(OtherActivity.this, "Cannot restore, all items are unchecked!", Toast.LENGTH_LONG)
                        .show();
            } else {

                mRecoverPhotosAsyncTask = new RecoverOthersAsyncTask(OtherActivity.this, adapter.getSelectedItem(),
                        new RecoverOthersAsyncTask.OnRestoreListener() {
                            @Override
