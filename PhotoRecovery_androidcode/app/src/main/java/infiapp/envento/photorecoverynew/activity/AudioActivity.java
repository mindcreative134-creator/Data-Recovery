...
import infiapp.envento.photorecoverynew.adapter.AudioAdapter;
import infiapp.envento.photorecoverynew.task.RecoverAudiosAsyncTask;
...
public class AudioActivity extends AppCompatActivity {
    ...
    private void gotonext() {
        ...
        if (status.equals("restore")) {
            ...
            if (tempList.size() == 0) {
                Toast.makeText(AudioActivity.this, "Cannot restore, all items are unchecked!", Toast.LENGTH_LONG)
                        .show();
            } else {

                mRecoverPhotosAsyncTask = new RecoverAudiosAsyncTask(AudioActivity.this, adapter.getSelectedItem(),
                        new RecoverAudiosAsyncTask.OnRestoreListener() {
                            @Override
