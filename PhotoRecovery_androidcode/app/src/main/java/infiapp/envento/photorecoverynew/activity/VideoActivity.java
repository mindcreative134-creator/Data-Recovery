...
import infiapp.envento.photorecoverynew.adapter.VideoAdapter;
import infiapp.envento.photorecoverynew.task.RecoverVideosAsyncTask;
...
public class VideoActivity extends AppCompatActivity {
    ...
    private void gotonext() {
        ...
        if (status.equals("restore")) {
            ...
            if (tempList.size() == 0) {
                Toast.makeText(VideoActivity.this, "Cannot restore, all items are unchecked!", Toast.LENGTH_LONG)
                        .show();
            } else {

                recoverVideosAsyncTask = new RecoverVideosAsyncTask(VideoActivity.this, adapter.getSelectedItem(),
                        new RecoverVideosAsyncTask.OnRestoreListener() {
                            @Override
