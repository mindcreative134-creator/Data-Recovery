...
import infiapp.envento.photorecoverynew.adapter.AlbumsImageAdapter;
        ...
        if (albumPhotos.size() != 0) {
            for (int i = 0; i < albumPhotos.size(); i++) {
                ...
                if (new File(temp).isHidden()) {
                    ...
                    Collections.sort(albumPhotos.get(i).getListPhoto(),
                            (lhs, rhs) -> Long.valueOf(rhs.getLastModified()).compareTo(lhs.getLastModified()));
